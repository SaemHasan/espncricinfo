...
def playersingle(request):
    ...
    for r in result:
        ...
        row = {'id': id, 'first_name': first_name, 'last_name': last_name, 'full_name': full_name,
               'nationality': nationality, 'date_of_birth': dob, 'image_link': image_link,'role': role, 'batstyle':bat_style,'bowlstyle':bowl_style}
    ...
    for r in res:
        ...
        row = {'mid': mid, 'scored_run': scored_run, 'balls': balls, 'fours': fours, 'sixes': sixes, 'notout': notout, 'overs': overs, 'givenrun': givenrun, 'wickets': wickets,
               'team1':team1, 'team2': team2, 'date':matchdate}
        Matches.append(row)

    return render(request, 'playersingle/playersingle.html', {'name': name, 'details': dict_result[0], 'playerstat': playerSTAT, 'matches': Matches})
